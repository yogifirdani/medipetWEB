                                    ...
                                    <tr rowId="{{ $id }}">
                                        <th>
                                            <input type="checkbox" name="selected_items[]" value="{{ $id }}"
                                                data-price="{{ $details['harga'] * $details['quantity'] }}" class="item-checkbox">
                                        </th>
                                        <th data-th="Product">
                                            <div class="row">
                                                <div class="col-sm-3 hidden-xs">
                                                    <img src="{{ asset('product/' . $details['image']) }}" class="card-img-top">
                                                </div>
                                                <div class="col-md-3 col-lg-3 col-xl-3">
                                                    <p class="nomargin" style="font-weight: bold; font-size: 150%;">{{ $details['nama'] }}</p>
                                                    <p data-th="harga">Rp.{{ $details['harga'] }}</p>
                                                </div>
                                                <th>
                                                    <div class="col-md-3 col-lg-6 col-xl d-flex">
                                                        <button data-mdb-button-init data-mdb-ripple-init class="btn btn-link px-2" onclick="updateQuantity(this, -1, {{ $details['harga'] }}, {{ $id }})">
                                                            <i class="fas fa-minus"></i>
                                                        </button>
                                                        <input id="quantity-{{ $id }}" min="1" name="quantity" value="{{ $details['quantity'] }}" type="number" class="form-control form-control-sm" onchange="updateTotalPrice()"/>
                                                        <button data-mdb-button-init data-mdb-ripple-init class="btn btn-link px-2" onclick="updateQuantity(this, 1, {{ $details['harga'] }}, {{ $id }})">
